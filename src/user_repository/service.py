...
def sync_user_repositories(
    ...
    """
    repo_names = [r.repo_full_name for r in user_repository_req_list]
...
def get_all_selected_repositories(user_id: int, db: Session):
    ...
    for user_repo in user_repositories:
        user_repository_res = UserRepositoryRes(repo_full_name=user_repo.repo_full_name)
...
def get_all_repositories_from_github(user_id: int, db: Session):
    ...
    return [UserRepositoryRes(repo_full_name=repo["name"]) for repo in repo_list]
