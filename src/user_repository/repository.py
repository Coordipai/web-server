...
def sync_user_repositories(db: Session, user_id: int, repo_names: list[str]) -> None:
    try:
        ...
        existing_repo_names = {repo.repo_full_name for repo in existing_repos}
        ...
        to_delete = [
            repo for repo in existing_repos if repo.repo_full_name not in new_repo_names
        ]
        ...
        for repo_name in to_add:
            new_repo = UserRepository(user_id=user_id, repo_full_name=repo_name)
            db.add(new_repo)
